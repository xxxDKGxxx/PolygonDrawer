...
public partial class PolygonDrawer : Form
{
    ...
    public PolygonDrawer()
    {
        ...
        InitializeCanvas();
        SetButtonEvents();
        _ = LoadPolygonFromJson("predefined-scene.json");
    }
    ...
    private void SetButtonEvents()
    {
        ...
        verticalRadioButton.CheckedChanged += (s, e) => ChangeEdge<VerticalEdge>(verticalRadioButton);

        obliqueRadioButton.CheckedChanged += (s, e) => ChangeEdge<Deg45Edge>(obliqueRadioButton);

        normalRadioButton.CheckedChanged += (s, e) => ChangeEdge<Edge>(normalRadioButton);

        fixedRadioButton.CheckedChanged += (s, e) => ChangeEdge<FixedLengthEdge>(fixedRadioButton);
        ...
        bezierRadioButton.CheckedChanged += (s, e) => ChangeEdge<BezierEdge>(bezierRadioButton);

        circularRadioButton.CheckedChanged += (s, e) => ChangeEdge<CircleEdge>(circularRadioButton);

        g0RadioButton.CheckedChanged += (s, e) => SetContinuuity(g0RadioButton, ContinuuityType.G0);

        g1RadioButton.CheckedChanged += (s, e) => SetContinuuity(g1RadioButton, ContinuuityType.G1);
    ...
    private void MainCanvas_MouseDown(object? sender, MouseEventArgs e)
    {
        ...
        {
            _selectedEdge = _polygon.GetEdgeByPoints(_selectedVert, otherVert);
        }
        ...
        g1RadioButton.Enabled = _selectedVert is not null
            && !_polygon.GetEdgesByPoint(_selectedVert)
                .Any(e => e is CircleEdge ce
                    && ce.GetPoints()
                        .Any(p => p != _selectedVert && p.Type == ContinuuityType.G1));
    ...
    {
        var (x, y) = _polygon.GetCenter();

        if (x > mainCanvas.Width || y > mainCanvas.Height)
        {
            _polygon.Translate(
                mainCanvas.Width / 2 - x,
                mainCanvas.Height / 2 - y);
        }
    ...
    private void DeserializeToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        var flowControl = LoadPolygonFromJson(path);
    ...
    private bool LoadPolygonFromJson(string path)
    {
        ...
        if (polygon is null)
        {
            _ = MessageBox.Show("Serialization failed.", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }
